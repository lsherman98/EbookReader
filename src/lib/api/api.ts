...
export const getChaptersByBookId = async (id?: string) => {
    ...
    return await pb.collection(Collections.Chapters).getFullList({
        filter: `book="${id}" && user="${getUserId()}"`,
...
export const getMessagesByChatId = async (id?: string) => {
    if (!getUserId() || !id) return;
    return await pb.collection(Collections.Chats).getOne<ChatsResponse<ExpandMessages>>(id, {
        expand: "messages",
    });
};
...
export const addMessage = async (
    ...
    if (!getUserId()) return;
    return await pb.collection(Collections.Messages).create({
        chat: id,
        content,
        role,
        user: getUserId(),
...
export const generateAIResponse = async (bookId: string, chatId: string, chapterId?: string) => {
    if (!getUserId()) return;
